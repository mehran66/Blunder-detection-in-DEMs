function [Current_Point]=NeigborPoint_Fnc(Surface,Current_Point,NumberOfPoint)
...
for ii=1:size(Current_Point,1)
    ...
    PointNum(ii,:)=Temp(index(1:1*coromozom_length),1);
    W1=t1(1:1*coromozom_length);
...
NewPop=NewPop(Index,:);
Neigbor_Point=NewPop(1,:);
if Cost_Fcn(Surface,Neigbor_Point)<Cost_Fcn(Surface,Current_Point)
    
    Current_Point=Neigbor_Point;
else
   Current_Point=0;
end
